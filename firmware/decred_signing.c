...
} decred_signing_stage;
static uint32_t idx1, idx2;
static uint32_t signatures;
static DecredTxRequest resp;
static DecredTxInType input;
static TxOutputBinType bin_output;
static DecredTxStruct to, tp, ti;
static state256 hashers[3];
...
The STAGE_ constants describe the decred_signing_stage when request is sent.
...
static void send_req_1_input(void)
{
	decred_signing_stage = STAGE_REQUEST_1_INPUT;
	resp.has_request_type = true;
	resp.request_type = RequestType_TXINPUT;
	resp.has_details = true;
	resp.details.has_request_index = true;
	resp.details.request_index = idx1;
	msg_write(MessageType_MessageType_DecredTxRequest, &resp);
}

static void send_req_2_prev_meta(void)
{
	decred_signing_stage = STAGE_REQUEST_2_PREV_META;
	resp.has_request_type = true;
...
static void send_req_2_prev_input(void)
{
	decred_signing_stage = STAGE_REQUEST_2_PREV_INPUT;
	resp.has_request_type = true;
	resp.request_type = RequestType_TXINPUT;
	resp.has_details = true;
	resp.details.has_request_index = true;
...
static void send_req_2_prev_output(void)
{
	decred_signing_stage = STAGE_REQUEST_2_PREV_OUTPUT;
	resp.has_request_type = true;
...
static void send_req_2_prev_extradata(uint32_t chunk_offset, uint32_t chunk_len)
{
	decred_signing_stage = STAGE_REQUEST_2_PREV_EXTRADATA;
	resp.has_request_type = true;
...
static void send_req_3_output(void)
{
	decred_signing_stage = STAGE_REQUEST_3_OUTPUT;
	resp.has_request_type = true;
	resp.request_type = RequestType_TXOUTPUT;
	resp.has_details = true;
	resp.details.has_request_index = true;
...
static void send_req_4_input(void)
{
	decred_signing_stage = STAGE_REQUEST_4_INPUT;
	resp.has_request_type = true;
	resp.request_type = RequestType_TXINPUT;
	resp.has_details = true;
	resp.details.has_request_index = true;
...
static void send_req_4_output(void)
{
	decred_signing_stage = STAGE_REQUEST_4_OUTPUT;
	resp.has_request_type = true;
	resp.request_type = RequestType_TXOUTPUT;
	resp.has_details = true;
	resp.details.has_request_index = true;
...
static void send_req_segwit_input(void)
{
	decred_signing_stage = STAGE_REQUEST_SEGWIT_INPUT;
	resp.has_request_type = true;
	resp.request_type = RequestType_TXINPUT;
	resp.has_details = true;
	resp.details.has_request_index = true;
...
static void send_req_segwit_witness(void)
{
	decred_signing_stage = STAGE_REQUEST_SEGWIT_WITNESS;
	resp.has_request_type = true;
	resp.request_type = RequestType_TXINPUT;
	resp.has_details = true;
	resp.details.has_request_index = true;
...
static void send_req_5_output(void)
{
	decred_signing_stage = STAGE_REQUEST_5_OUTPUT;
	resp.has_request_type = true;
	resp.request_type = RequestType_TXOUTPUT;
	resp.has_details = true;
	resp.details.has_request_index = true;
...
static void send_req_finished(void)
{
	resp.has_request_type = true;
...
static void phase1_request_next_input(void)
{
	if (idx1 < inputs_count - 1) {
		...
		blake256_final(&hashers[0], hash_prevouts);
		blake256_hash(hash_prevouts, hash_prevouts, 32);

		blake256_final(&hashers[1], hash_sequence);
		blake256_hash(hash_sequence, hash_sequence, 32);
		blake256_final(&hashers[2], hash_check);
		// init hashOutputs
		blake256_init(&hashers[0]);
		idx1 = 0;
		send_req_3_output();
	}
}

static void phase2_request_next_input(void)
{
...
static bool decred_compile_input_script_sig(DecredTxInType *tinput)
{
...
void decred_signing_init(uint32_t _inputs_count, uint32_t _outputs_count, const CoinType *_coin, const HDNode *_root, uint32_t _version, uint32_t _lock_time)
{
	...
	memset(&input, 0, sizeof(DecredTxInType));
	memset(&resp, 0, sizeof(DecredTxRequest));
	...
	decred_tx_init(&to, inputs_count, outputs_count, version, lock_time, 0, false);
	// segwit hashes for hashPrevouts and hashSequence
	blake256_init(&hashers[0]);
	blake256_init(&hashers[1]);
	blake256_init(&hashers[2]);
...
static bool signing_check_input(DecredTxInType *txinput) {
	...
	decred_extract_input_bip32_path(txinput);
	// compute segwit hashPrevouts & hashSequence
	decred_tx_prevout_hash(&hashers[0], txinput);
	decred_tx_sequence_hash(&hashers[1], txinput);
	// hash prevout and script type to check it later (relevant for fee computation)
	decred_tx_prevout_hash(&hashers[2], txinput);
	blake256_update(&hashers[2], (const uint8_t *) &txinput->script_type, sizeof(&txinput->script_type));
	return true;
}
...
static bool signing_check_prevtx_hash(void) {
	uint8_t hash[32];
	decred_tx_hash_final(&tp, hash, true);
...
static bool decred_signing_check_output(DecredTxOutType *txoutput) {
	...
	if (txoutput->address_n_count > 0) {
		...
		if (txoutput->script_type == OutputScriptType_PAYTOMULTISIG) {
			...
			if (multisig_fp_set && !multisig_fp_mismatch
						...
						&& memcmp(multisig_fp, h, 32) == 0) {
				is_change = decred_check_change_bip32_path(txoutput);
			}
	...
	int co = decred_compile_output(coin, root, txoutput, &bin_output, !is_change);
	if (!is_change) {
	...
	decred_tx_output_hash(&hashers[0], &bin_output);
	return true;
}
...
static void phase1_request_next_output(void) {
	if (idx1 < outputs_count - 1) {
		...
		blake256_final(&hashers[0], hash_outputs);
		blake256_hash(hash_outputs, hash_outputs, 32);
...
static bool signing_sign_input(void) {
	...
	decred_tx_hash_final(&ti, hash, false);
	...
	resp.serialized.serialized_tx.size = decred_tx_serialize_input(&to, &input, resp.serialized.serialized_tx.bytes);
	return true;
}
...
bool decred_signing_sign_segwit_input(DecredTxInType *txinput) {
	...
	if (txinput->script_type == InputScriptType_SPENDWITNESS
		...
		if (!decred_compile_input_script_sig(txinput)) {
		...
		blake256_init(&hashers[0]);
		blake256_update(&hashers[0], (const uint8_t *)&version, 4);
		blake256_update(&hashers[0], hash_prevouts, 32);
		blake256_update(&hashers[0], hash_sequence, 32);
		decred_tx_prevout_hash(&hashers[0], txinput);
		decred_tx_script_hash(&hashers[0], txinput->script_sig.size, txinput->script_sig.bytes);
		blake256_update(&hashers[0], (const uint8_t*) &txinput->amount, 8);
		decred_tx_sequence_hash(&hashers[0], txinput);
		blake256_update(&hashers[0], hash_outputs, 32);
		blake256_update(&hashers[0], (const uint8_t*) &lock_time, 4);
		blake256_update(&hashers[0], (const uint8_t*) &sighash, 4);
		blake256_final(&hashers[0], hash);
		blake256_hash(hash, hash, 32);
	...
	if (idx1 == inputs_count - 1) {
		...
		r += decred_tx_serialize_footer(&to, resp.serialized.serialized_tx.bytes + r);
		resp.serialized.serialized_tx.size = r;
	}
	return true;
}
	...
	switch (decred_signing_stage) {
		case STAGE_REQUEST_1_INPUT:
			...
			if (tx->inputs[0].script_type == InputScriptType_SPENDMULTISIG
				...
				memcpy(&input, tx->inputs, sizeof(DecredTxInType));
		...
		case STAGE_REQUEST_2_PREV_META:
			decred_tx_init(&tp, tx->inputs_cnt, tx->outputs_cnt, tx->version, tx->lock_time, tx->extra_data_len, false);
			progress_meta_step = progress_step / (tp.inputs_len + tp.outputs_len);
			idx2 = 0;
			if (tp.inputs_len > 0) {
				send_req_2_prev_input();
			} else {
				decred_tx_serialize_header_hash(&tp);
		...
		case STAGE_REQUEST_2_PREV_INPUT:
			...
			if (!decred_tx_serialize_input_hash(&tp, tx->inputs)) {
		...
		case STAGE_REQUEST_2_PREV_OUTPUT:
			...
			if (!decred_tx_serialize_output_hash(&tp, tx->bin_outputs)) {
		...
		case STAGE_REQUEST_2_PREV_EXTRADATA:
			if (!decred_tx_serialize_extra_data_hash(&tp, tx->extra_data.bytes, tx->extra_data.size)) {
		...
		case STAGE_REQUEST_3_OUTPUT:
			if (!decred_signing_check_output(&tx->outputs[0])) {
				return;
			}
		...
		case STAGE_REQUEST_4_INPUT:
			...
			if (idx2 == 0) {
				decred_tx_init(&ti, inputs_count, outputs_count, version, lock_time, 0, true);
				blake256_init(&hashers[0]);
			}
			// check prevouts and script type
			decred_tx_prevout_hash(&hashers[0], tx->inputs);
			blake256_update(&hashers[0], (const uint8_t *) &tx->inputs[0].script_type, sizeof(&tx->inputs[0].script_type));
			if (idx2 == idx1) {
				if (!decred_compile_input_script_sig(&tx->inputs[0])) {
					fsm_sendFailure(FailureType_Failure_Other, "Failed to compile input");
					decred_signing_abort();
					return;
				}
			...
			if (!decred_tx_serialize_input_hash(&ti, tx->inputs)) {
				fsm_sendFailure(FailureType_Failure_Other, "Failed to serialize input");
				decred_signing_abort();
				return;
			}
		...
		case STAGE_REQUEST_4_OUTPUT:
			...
			if (decred_compile_output(coin, root, tx->outputs, &bin_output, false) <= 0) {
				fsm_sendFailure(FailureType_Failure_Other, "Failed to compile output");
				decred_signing_abort();
				return;
			}
			//  check hashOutputs
			decred_tx_output_hash(&hashers[0], &bin_output);
			if (!decred_tx_serialize_output_hash(&ti, &bin_output)) {
				fsm_sendFailure(FailureType_Failure_Other, "Failed to serialize output");
				decred_signing_abort();
				return;
			}
		...
		case STAGE_REQUEST_SEGWIT_INPUT:
			...
			if (tx->inputs[0].script_type == InputScriptType_SPENDP2SHWITNESS
				...
				if (!decred_compile_script_multisig_hash(&tx->inputs[0].multisig, tx->inputs[0].script_sig.bytes + 3)) {
					fsm_sendFailure(FailureType_Failure_Other, "Failed to compile input");
					decred_signing_abort();
					return;
				}
			...
			resp.serialized.serialized_tx.size = decred_tx_serialize_input(&to, &tx->inputs[0], resp.serialized.serialized_tx.bytes);
			update_ctr = 0;
			if (idx1 < inputs_count - 1) {
				idx1++;
				phase2_request_next_input();
		...
		case STAGE_REQUEST_5_OUTPUT:
			...
			resp.has_serialized = true;
			resp.serialized.has_serialized_tx = true;
			resp.serialized.serialized_tx.size = decred_tx_serialize_output(&to, &bin_output, resp.serialized.serialized_tx.bytes);
			if (idx1 < outputs_count - 1) {
				idx1++;
				send_req_5_output();
		...
		case STAGE_REQUEST_SEGWIT_WITNESS:
			if (!decred_signing_sign_segwit_input(&tx->inputs[0])) {
				return;
			}
			signatures++;
			progress = 500 + ((signatures * progress_step) >> PROGRESS_PRECISION);
